...
To mirror the form of the paper's Fig.~22 \cite{ding2017}, Figure~\ref{fig:sndrconv} plots the SNDR obtained by evaluating the implemented DAC-mismatch equations and the implemented two-mode comparator model at the trim states recorded during the background-calibration run. The plotted curve is generated directly from those sampled states and then smoothed for presentation so the long-term convergence trend is easier to read. After convergence, the evaluated SNDR reaches approximately \SI{73.9}{dB}.
