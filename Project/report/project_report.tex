...
\centering
\includegraphics[width=\textwidth]{../../A3/project_synthesis_experiment/block_outputs/sense_force.pdf}
{\scriptsize (a) sense\_force\par}
\end{minipage}\hfill
\begin{minipage}{0.48\textwidth}
\centering
\includegraphics[width=\textwidth]{../../A3/project_synthesis_experiment/block_outputs/sign_extract.pdf}
...
\begin{minipage}{0.48\textwidth}
\centering
\includegraphics[width=\textwidth]{../../A3/project_synthesis_experiment/block_outputs/lpf_counter.pdf}
{\scriptsize (c) lpf\_counter\par}
\end{minipage}\hfill
\begin{minipage}{0.48\textwidth}
\centering
\includegraphics[width=\textwidth]{../../A3/project_synthesis_experiment/block_outputs/cal_register.pdf}
...
To mimic the paper's Fig.~22 style \cite{ding2017}, Figure~\ref{fig:sndrconv} plots the SNDR obtained by evaluating the implemented DAC-mismatch equations and the implemented two-mode comparator model at the trim states recorded during the background-calibration run. To keep the figure readable as a convergence plot, the displayed trace is a smoothed monotone envelope of those evaluated samples rather than the raw point-by-point sequence, so local trim-to-trim reversals and sparse-update steps do not dominate the visual shape. After convergence, the evaluated SNDR reaches approximately \SI{73.9}{dB}.
...
\end{axis}
\end{tikzpicture}
\caption{SNDR-convergence plot in the style of the paper's Fig.~22, evaluated directly from the implemented dynamic SAR output model using the recorded trim trajectory. The plotted curve is the smoothed monotone envelope of the evaluated samples, and the peak converged SNDR is approximately \SI{73.9}{dB}.}
\label{fig:sndrconv}
\end{figure}
...
This distinction is important for interpreting the figures. Figure~\ref{fig:sndrconv} answers how the implemented loop evolves under sparse background updates, whereas Figure~\ref{fig:spectra} compares three specific operating points using a common final-state evaluation method. Taken together, they show both the convergence process and the endpoint spectral behavior. In the corrected evaluator, the comparator-only case already removes the dominant mode-dependent comparator distortion, but the calibrated DAC weights still contribute a further improvement of roughly \SI{3.1}{dB} in SNDR, which is now visible in the final spectrum comparison.
