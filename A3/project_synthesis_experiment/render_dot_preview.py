...
DOT_PATH = ROOT / "project_calibration_digital_top.dot"
OUT_PATH = ROOT / "project_calibration_digital_top_preview.png"
...
def initial_positions(nodes: dict[str, dict[str, str]]):
    ...
    x_map = {"port": 0.0, "state": 1.1, "cell": 2.7, "slice": 4.3, "other": 5.1}
    pos = {}
...
def fruchterman_reingold(nodes, edges, pos, iterations=260):
    ids = list(nodes.keys())
    n = max(len(ids), 1)
    area = 16.0
    k = math.sqrt(area / n)
...
def render(nodes, edges, pos, out_path: Path):
    ...
    ax.set_title("Synthesized Project Calibration Digital Path", fontsize=14)
    ax.text(
        0.0,
        -1.42,
